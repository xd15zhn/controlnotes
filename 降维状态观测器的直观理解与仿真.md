...
将原系统封装成组合模块，输入为$u$，输出的第0个端口为$y$，第1到3个端口分别为$x_1$至$x_3$。直接拆开写成积分器的形式太过复杂，可以使用状态空间组合模块，但是输出有4个，C矩阵的最后三行为3阶单位阵。定义A、B、C矩阵和原系统
```cpp
    ...
    Mat C2 = eye(3);
    Mat C = VConcat(C1, C2);
    auto Model = new StateSpace(&sim1, A, B, C, Mat());  // 原系统
```
定义降维状态观测器
```cpp
    ...
    C = eye(2);
    auto Observer = new StateSpace(&sim1, A, B, C);  // 降维状态观测器
```
降维状态观测器的输入为$[y,u]^{\text{T}}$，输出为$[z_1,z_2]^{\text{T}}$。
定义状态重构
```cpp
    A = Mat(3, 3, vecdble{2, -1, 0, -1, 1, 0, -3, 0, 1});
    auto Restructer = new StateGain(&sim1, A);  // 状态重构器
```
连接各个模块
```cpp
    sim1.connect(min1, Model);  // 输入信号u接入原系统
    sim1.connect(Model, 0, Observer, 0);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connect(min1, Observer, 1);  // 输入信号u接入状态观测器的第2个输入端口
    sim1.connect(Model, Restructer, 0);  // 连接原系统的输出y与状态重构器的第1个输入端口
    sim1.connect(Observer, 0, Restructer, 1);  // 连接状态观测器的输出z1与状态重构器的第2个输入端口
    sim1.connect(Observer, 1, Restructer, 2);  // 连接状态观测器的输出z2与状态重构器的第3个输入端口
```
...
int main()
{
    ...
    Mat C2 = eye(3);
    Mat C = VConcat(C1, C2);
    auto Model = new StateSpace(&sim1, A, B, C);  // 原系统
    A = Mat(2, 2, vecdble{-3, 0, -7, -4});
    B = Mat(2, 2, vecdble{2, 2, 17, 7});
    C = eye(2);
    auto Observer = new StateSpace(&sim1, A, B, C);  // 降维状态观测器
    ...
    FMOutput(mx1, &sim1);  // 原系统输出x1
    FMOutput(mx1p, &sim1);  // 预测输出x1p
    // FMOutput(mx2, &sim1);  // 原系统输出x2
    // FMOutput(mx2p, &sim1);  // 预测输出x2p
    // FMOutput(mx3, &sim1);  // 原系统输出x3
    // FMOutput(mx3p, &sim1);  // 预测输出x3p
    sim1.connect(Model, 1, mx1);
    sim1.connect(Restructer, 0, mx1p);
    min1->Set_Function([](double t){return sin(t)+1;});
    Model->Set_InitialValue(vecdble{1, 1, 1});  // 将原系统的初始状态全部设置为1
...
int main()
{
    ...
    Simulator sim1(10);
    auto mtf1 = new StateSpace(&sim1, A, B, C, Mat());
    FMInput(min1, &sim1);
    FMOutput(mout1, &sim1);
    sim1.connect(min1, mtf1);
