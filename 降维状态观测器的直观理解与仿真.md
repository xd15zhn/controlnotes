...
int main()
{
    ...
    Mat C2 = eye(3);
    Mat C = VConcat(C1, C2);
    auto Model = new StateTransFcn(&sim1, A, B, C, Mat(), true, "stfmdl");  // 原系统
    A = Mat(2, 2, vecdble{-3, 0, -7, -4});
    B = Mat(2, 2, vecdble{2, 2, 17, 7});
    C = eye(2);
    auto Observer = new StateTransFcn(&sim1, A, B, C, Mat(), true, "stfobserver");  // 降维状态观测器
    A = Mat(3, 3, vecdble{2, -1, 0, -1, 1, 0, -3, 0, 1});
    ...
    sim1.connectU(uin1, mxModel);  // 输入信号u单线接入原系统的输入复用器
    sim1.connectM(mxModel, Model);  // 原系统的输入复用器以总线接入原系统
    sim1.connectM(Model, dmxModel);  // 原系统通过输出解复用器引出单线
    sim1.connectU(dmxModel, 0, mxObserver, 0);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connectU(uin1, mxObserver, 1);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connectM(mxObserver, Observer);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connectM(Observer, dmxObserver);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connectU(dmxModel, mxRestructer, 0);  // 连接原系统的输出y与状态重构器的第1个输入端口
    sim1.connectU(dmxObserver, 0, mxRestructer, 1);  // 连接状态观测器的输出z1与状态重构器的第2个输入端口
    sim1.connectU(dmxObserver, 1, mxRestructer, 2);  // 连接状态观测器的输出z2与状态重构器的第3个输入端口
    sim1.connectM(mxRestructer, Restructer);  // 连接原系统的输出y与状态观测器的第1个输入端口
    sim1.connectM(Restructer, dmxRestructer);  // 连接原系统的输出y与状态观测器的第1个输入端口
    ...
    sim1.connectU(dmxModel, 1, ux1);
    sim1.connectU(dmxRestructer, 0, ux1p);
    uin1->Set_Function([](double t){return sin(t)+1;});
    Model->Set_InitialValue(vecdble{1, 1, 1});  // 将原系统的初始状态全部设置为1
    sim1.Initialize();
    sim1.Simulate();
    sim1.Plot();
